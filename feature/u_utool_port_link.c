...
#define UTOOL_MAX_TIME_INFO_SIZE 64U
...
int utool_port_link_parse_rpc_pkt(struct fwctl_rpc_ub_out *out)
{
	...
	if ((out == NULL) || (out->data_size < sizeof(struct ubctl_port_link_stats))) {
		utool_err_msg("Param is invalid, out==null(%d) or data size = %ubytes is less than the struct size %zubytes(%d).\n",
			      ...
			      sizeof(struct ubctl_port_link_stats),out == NULL ? 0 : 1);
	...
	if (data_num > UTOOL_MAX_PORT_LINK_STATUS) {
		utool_err_msg("Data num = %u is bigger than maximum %u.\n", data_num, UTOOL_MAX_PORT_LINK_STATUS);
		return UTOOL_ERR_INVALID_PARAM;
	}
