...
#define UTOOL_FW_HIGH_OFFSET 16
	...
	if (fw_version_out->data_size != sizeof(struct utool_fw_version_data)) {
		utool_err_msg("Failed to parse firmware version rpc pkt, out data size = %ubytes, it must be %zubytes.\n",
			      fw_version_out->data_size, sizeof(struct utool_fw_version_data));
		return UTOOL_ERR_INVALID_PARAM;
	}
